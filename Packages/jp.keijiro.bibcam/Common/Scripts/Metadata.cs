...
public readonly struct Metadata
{
    ...
    public Metadata(Transform camera, in Matrix4x4 projection, Vector2 range, Transform experienceOrigin)
    {
        var p = camera.position;
        var r = camera.rotation;
        var rsign = r.w < 0 ? -1.0f : 1.0f;

        var ap = experienceOrigin.position;
        var ar = experienceOrigin.rotation;
        var arsign = ar.w < 0 ? -1.0f : 1.0f;
